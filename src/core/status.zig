...
pub fn showStatus(allocator: std.mem.Allocator, repo: *repository.Repository) !void {
    ...
    while (try iterator.next()) |entry| {
        ...
        const file_data = std.fs.cwd().readFileAlloc(entry.name, allocator, @enumFromInt(10 * 1024 * 1024)) catch continue;
        defer allocator.free(file_data);

        const file_cid = cid.CID.fromBytes(file_data);
        const in_store = try repo.store.has(file_cid);
